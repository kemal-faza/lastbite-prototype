...
export function QueueIndicator({ initialQueue = 3, storeName }: QueueIndicatorProps) {
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-gray-900 text-sm">Antrean Saat Ini</h3>
            <p className="text-xs text-gray-500">
              {storeName ? storeName : 'Antrean pickup'} — {currentQueue} orang
            </p>
          </div>
        </div>
        <div className="text-right">
          <div className="flex items-center gap-1 text-[var(--destructive)]">
            ...
            <span className="text-sm font-bold">{estimatedTime} menit</span>
          </div>
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <div
            ...
            className={
              ...
              (i < currentQueue ? 'bg-[var(--secondary)]' : 'bg-gray-100')
            }
          />
        ))}
      </div>
