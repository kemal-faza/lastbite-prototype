...
import { Minus, Plus, Trash2, ShoppingBag, Tag, ArrowRight, ChevronLeft, Check, User, ClipboardList } from 'lucide-react';
...
export function Cart() {
        ...
        {STEPS.map((label, i) => {
          ...
          return (
            <div key={label} className="flex items-center">
              ...
              {i < STEPS.length - 1 && (
                <div className={'w-12 h-0.5 mx-2 mb-5 ' + (isDone ? 'bg-[var(--primary)]' : 'bg-gray-200')} />
              )}
            </div>
          );
        })}
    ...
    <div className="p-4 space-y-4">
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        {items.map((item, index) => (
          <div
            key={item.id}
            className={'p-4 flex gap-4' + (index !== items.length - 1 ? ' border-b border-gray-100' : '')}
          >
            <img
              src={item.image}
              alt={item.name}
  ...
  const renderConfirmStep = () => (
    <div className="p-4 space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm p-4 border border-gray-100">
        <h3 className="font-semibold text-gray-900 mb-3">Data Pemesan</h3>
  ...
  return (
    <div className="flex flex-col min-h-full bg-[var(--background)] pb-28">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="px-4 py-4 flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold text-gray-900">
            {checkoutStep === 1 ? 'Keranjang' : checkoutStep === 2 ? 'Pembayaran' : 'Konfirmasi'}
          </h1>
        </div>
        {renderStepIndicator()}
      </div>
      ...
      {/* Bottom CTA */}
      <div className="fixed bottom-[72px] left-0 right-0 bg-white border-t border-gray-200 p-4 z-40 max-w-md mx-auto shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)]">
        {renderBottomBar()}
      </div>
    </div>
  );
